...
public class TerminalWebSocketHandler extends TextWebSocketHandler {
    ...
    private String shellDoubleQuoteContent(String value) {
        return value.replace("\\", "\\\\")
                .replace("\"", "\\\"")
                .replace("$", "\\$")
                .replace("`", "\\`");
    ...
    private String codexCliBinDir() {
        String binDir = appProperties.codexCliBinDir();
        return binDir == null || binDir.isBlank() ? "/usr/local/bin" : binDir.trim();
    }
    ...
    private String codexModel() {
        ...
        String openAiModel = appProperties.openAiModel();
        return openAiModel == null || openAiModel.isBlank() ? "codex" : openAiModel.trim();
    }

    private String buildCodexCommand() {
        String codexPath = shellQuote(codexCliPath());
        return """
                if [ ! -x %s ]; then
                  echo 'Codex CLI is not installed at the configured APP_CODEX_CLI_PATH.'
                  exec sh -il
                fi
                exec %s -m %s
                """.formatted(codexPath, codexPath, shellQuote(codexModel()));
    }

    private String codexSessionEnvironment(String username) {
        return chatCredentialService.resolveOpenAiApiKey(username)
                .map(apiKey -> " OPENAI_API_KEY=" + shellQuote(apiKey) + " CODEX_MODEL=" + shellQuote(codexModel()))
    ...
    private String buildCliBootstrap(Path workspaceHome) {
        ...
        String profile = """
                export PATH="$PATH:/usr/local/bin:/usr/bin:%s"
                ...
                """.formatted(shellDoubleQuoteContent(codexCliBinDir()), shellQuote(codexCliPath()));
