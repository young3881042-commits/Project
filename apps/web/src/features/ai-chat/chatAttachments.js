export const CHAT_ATTACHMENT_ACCEPT = '.pdf,.txt,.md,.markdown,.csv,.json,.log,application/pdf,text/plain,text/markdown,text/csv,application/json';
export const MAX_ATTACHMENTS = 3;
export const MAX_ATTACHMENT_CHARS = 12000;
export const MAX_ATTACHMENT_TOTAL_CHARS = 24000;
export const MAX_PDF_BYTES = 5 * 1024 * 1024;
export const MAX_TEXT_BYTES = 200 * 1024;
const extensions = new Set(['pdf', 'txt', 'md', 'markdown', 'csv', 'json', 'log']);
export function attachmentName(name) {
  ...
  if (!extensions.has(clean.split('.').at(-1).toLowerCase())) throw new Error('PDF·TXT·Markdown·CSV·JSON·LOG 파일을 첨부할 수 있어요.');
  return clean;
}
