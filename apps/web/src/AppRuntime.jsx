...
function schedulerRecurrenceForItem(item) {
  return scheduleRepeatFor(item);
}
...
export function addSchedulerItem(item) {
  ...
  const now = new Date();
  const recurrence = schedulerRecurrenceForItem(item);
  const date = item?.date || toDateKey(now);
  const nextItem = {
    ...
    repeat: recurrence,
    recurrence,
    repeatDays: recurrence === 'custom' ? normalizeRepeatDays(item?.repeatDays, date) : [],
...
function normalizeSchedulerItem(item) {
  const recurrence = schedulerRecurrenceForItem(item);
  const date = isDateKey(item?.date) ? item.date : toDateKey(new Date());
  return {
    ...item,
    date,
    type: normalizeSchedulerType(item?.type),
...
function isWorkoutProfileComplete(profile) {
  const normalizedProfile = normalizeWorkoutProfile(profile);
  return Boolean(
    parseWorkoutNumber(normalizedProfile.weightKg)
    && parseWorkoutNumber(normalizedProfile.heightCm)
    && parseWorkoutNumber(normalizedProfile.age)
  );
}

function readWorkoutProfileState(input = readStoredAuth()) {
  const storageKey = workoutProfileStorageKey(input);
  try {
    const scopedRaw = localStorage.getItem(storageKey);
    const legacyRaw = storageUsername(input) === 'guestuser'
      ? localStorage.getItem(WORKOUT_PROFILE_KEY)
      : null;
    const raw = scopedRaw ?? legacyRaw;
    const profile = normalizeWorkoutProfile(raw ? JSON.parse(raw) : DEFAULT_WORKOUT_PROFILE);
    if (scopedRaw === null && legacyRaw !== null) {
      localStorage.setItem(storageKey, JSON.stringify(profile));
      localStorage.removeItem(WORKOUT_PROFILE_KEY);
    }
    return { profile, saved: raw !== null && isWorkoutProfileComplete(profile) };
  } catch {
    return { profile: normalizeWorkoutProfile(DEFAULT_WORKOUT_PROFILE), saved: false };
  }
}
...
function saveWorkoutProfile(storageKey, profile) {
  ...
  if (storageKey.endsWith(':guestuser')) localStorage.removeItem(WORKOUT_PROFILE_KEY);
  return nextProfile;
}
...
function normalizeWorkoutLog(log, index = 0) {
  ...
  return {
    ...
    bodyAge: Math.max(0, Number(log?.bodyAge) || 0),
    bodySex: log?.bodySex === 'female' ? 'female' : 'male',
...
function readWorkoutLogs(input = readStoredAuth()) {
  const storageKey = workoutStorageKey(input);
  try {
    const scopedRaw = localStorage.getItem(storageKey);
    const legacyRaw = storageUsername(input) === 'guestuser'
    ...
    const parsed = JSON.parse(raw);
    if (!Array.isArray(parsed)) return [];
    const logs = parsed.map(normalizeWorkoutLog).filter(Boolean).sort(compareDatedRecords).slice(0, 120);
    if (scopedRaw === null && legacyRaw !== null) {
...
function saveWorkoutLogs(storageKey, logs) {
  ...
  if (storageKey.endsWith(':guestuser')) localStorage.removeItem(WORKOUT_LOGS_KEY);
  return nextLogs;
}
...
function isScheduledOnDate(item, dateKey) {
  return scheduleRecurrenceMatches(item, dateKey);
}
...
function normalizeNoteBlock(block) {
  ...
  const labels = [...new Set([...normalizeNoteLabels(block?.labels || tags), ...labelsForPlanType(planType)])];
  return {
    ...block,
...
export function AppAuthPage({ mode, navigate }) {
...
export function WorkspaceApp({ navigate }) {
...
export function AnalysisFileEditorPage({ navigate }) {
  const params = new URLSearchParams(window.location.search);
...
export function PortfolioHomePage({ navigate }) {
  useEffect(() => {
  ...
  const portfolioLinks = [
    {
      title: '메모 보드',
      detail: '폴더형 보드, BlockNote 편집, 체크리스트를 한 흐름으로 정리',
      action: '메모 열기',
      path: '/notes',
      tone: 'assistant',
      icon: 'board'
    ...
    {
      title: '일정 관리',
      detail: '오늘 일정, 주간 일정, 할 일 상태를 모바일 화면에서 바로 확인',
      action: '일정 보기',
      path: '/scheduler',
      tone: 'schedule',
      icon: 'calendar'
    ...
    {
      title: '운동·식단',
      detail: '운동 소모와 먹은 칼로리, 기초대사량을 오늘의 에너지 흐름으로 연결',
      action: '식단 열기',
      path: '/diet',
      tone: 'travel',
      icon: 'meal'
    ...
    {
      title: '가계부',
      detail: '수입과 지출을 빠르게 기록하고 월별 흐름을 모바일에서 확인',
      action: '가계부 열기',
      path: '/finance',
      tone: 'deploy',
      icon: 'chart'
    ...
    <main className="portfolioHome">
      <nav className="portfolioNav" aria-label="portfolio navigation">
        ...
        <div>
          ...
          <button type="button" onClick={() => navigate('/notes')}>메모</button>
          <button type="button" onClick={() => navigate('/scheduler')}>일정</button>
          <button type="button" onClick={() => navigate('/diet')}>식단</button>
        </div>
      </nav>
      <section className="portfolioHero">
        <div className="portfolioHeroCopy">
          <span className="portfolioEyebrow">Memo · Schedule · Wellness</span>
          <h1>기록에서 건강 관리까지 이어지는 개인 앱</h1>
          <p>
            메모와 체크리스트를 기록하고 날짜가 있는 항목은 일정으로 연결합니다.
            운동, 식단, 가계부를 로그인 없이 한 기기에서 빠르게 관리할 수 있습니다.
          </p>
          <div className="portfolioHeroActions">
            <button type="button" onClick={() => navigate('/app')}><MemoNavIcon type="spark" />앱 시작</button>
            <button type="button" onClick={() => navigate('/notes')}><MemoNavIcon type="board" />메모 열기</button>
          </div>
        </div>
        <div className="portfolioDevice" aria-label="app preview">
          <div className="portfolioDeviceTop">
            ...
            <small>Memo + Schedule</small>
          </div>
          <div className="portfolioDevicePhoto">
            <span>BlockNote 메모에서 일정까지</span>
          </div>
          ...
          <div className="portfolioPreviewList">
            ...
            <span>13:30 회의 메모를 할 일로 정리</span>
            <span>20:00 저녁 식단과 운동 기록</span>
          </div>
        </div>
      </section>
...
export function SpaceHomePage({ navigate }) {
...
export function MorePage({ navigate }) {
  const session = readStoredAuth();
  const extraFeatures = [
    {
      key: 'travel',
  ...
  return (
    ...
    >
      <section className="spaceAppFrame appHomeDashboard morePageDashboard" aria-label="더보기">
        <header className="moreDesktopIntro">
          <span>Workspace tools</span>
          <h1>더보기</h1>
          <p>여행, 가계부, 운동 기록을 한 워크스페이스에서 이어서 관리하세요.</p>
        </header>
...
export function BudgetPage({ navigate }) {
  const session = readStoredAuth();
...
export function WorkoutPage({ navigate }) {
  const session = readStoredAuth();
  ...
  const initialProfileState = useMemo(() => readWorkoutProfileState(session), [profileKey]);
  const [profile, setProfile] = useState(initialProfileState.profile);
  const [profileDraft, setProfileDraft] = useState(initialProfileState.profile);
  const [profileSaved, setProfileSaved] = useState(initialProfileState.saved);
  const [profileEditing, setProfileEditing] = useState(!initialProfileState.saved);
  const [statusText, setStatusText] = useState('');
  const [draft, setDraft] = useState(() => ({
  ...
  useEffect(() => {
    const nextProfileState = readWorkoutProfileState(readStoredAuth());
    setLogs(readWorkoutLogs(readStoredAuth()));
    setProfile(nextProfileState.profile);
    setProfileDraft(nextProfileState.profile);
    setProfileSaved(nextProfileState.saved);
    setProfileEditing(!nextProfileState.saved);
    setDraft({
      templateId: WORKOUT_TEMPLATES[0].id,
      date: today,
      startTime: '',
  ...
  const updateWorkoutProfile = (field, value) => {
    setProfileDraft((current) => normalizeWorkoutProfile({ ...current, [field]: value }));
  };

  const submitWorkoutProfile = (event) => {
    event.preventDefault();
    const nextProfile = normalizeWorkoutProfile(profileDraft);
    if (!isWorkoutProfileComplete(nextProfile)) {
      setStatusText('몸무게, 키, 나이를 모두 입력하세요.');
      return;
    }
    const savedProfile = saveWorkoutProfile(profileKey, nextProfile);
    setProfile(savedProfile);
    setProfileDraft(savedProfile);
    setProfileSaved(true);
    setProfileEditing(false);
    setStatusText('운동 프로필을 저장했습니다. 이후 운동 계산에 자동으로 사용합니다.');
  };

  const cancelWorkoutProfileEdit = () => {
  ...
  const submitWorkout = (event) => {
    ...
    if (!profileSaved) {
      ...
      setStatusText('운동 기록 전에 운동 프로필을 먼저 저장하세요.');
      return;
    }
    ...
    const nextLog = normalizeWorkoutLog({
      ...
      bodyAge: parseWorkoutNumber(profile.age),
      bodySex: profile.sex,
      memo: draft.memo,
      exercises,
      createdAt: new Date().toISOString()
  ...
  return (
    ...
    >
      <section className="spaceAppFrame appHomeDashboard utilityPageDashboard" aria-label="운동">
        ...
        <section className="utilityPageCard workoutProfileCard" aria-label="운동 프로필과 칼로리 계산">
          <header className="utilitySectionHeader">
            <strong>운동 프로필</strong>
            <small>{profileSaved ? '저장됨 · BMR/kcal 계산 기준' : '최초 1회 저장'}</small>
          </header>
          {profileSaved && !profileEditing ? (
            <div className="workoutProfileSummaryCard">
              <div>
                <strong>{profile.weightKg}kg · {profile.heightCm}cm · {profile.age}세 · {profile.sex === 'female' ? '여성' : '남성'}</strong>
                <small>운동 기록의 BMR과 예상 소모 칼로리는 이 프로필을 사용합니다.</small>
              </div>
              <button type="button" onClick={() => setProfileEditing(true)}>수정</button>
            </div>
          ) : (
            <form className="workoutProfileEditor" onSubmit={submitWorkoutProfile}>
              <div className="utilityFormGrid workoutProfileGrid">
                <label className="utilityField">
                  <span>몸무게(kg)</span>
                  <input type="number" min="0" step="0.1" inputMode="decimal" placeholder="예: 70" value={profileDraft.weightKg} onChange={(event) => updateWorkoutProfile('weightKg', event.target.value)} />
                </label>
                <label className="utilityField">
                  <span>키(cm)</span>
                  <input type="number" min="0" step="0.1" inputMode="decimal" placeholder="예: 175" value={profileDraft.heightCm} onChange={(event) => updateWorkoutProfile('heightCm', event.target.value)} />
                </label>
                <label className="utilityField">
                  <span>나이</span>
                  <input type="number" min="1" inputMode="numeric" value={profileDraft.age} onChange={(event) => updateWorkoutProfile('age', event.target.value)} />
                </label>
                <label className="utilityField">
                  <span>성별</span>
                  <select value={profileDraft.sex} onChange={(event) => updateWorkoutProfile('sex', event.target.value)}>
                    <option value="male">남성</option>
                    <option value="female">여성</option>
                  </select>
                </label>
              </div>
              <div className="workoutProfileActions">
                {profileSaved ? <button type="button" onClick={cancelWorkoutProfileEdit}>취소</button> : null}
                <button type="submit" className="primary">프로필 저장</button>
              </div>
            </form>
          )}
        ...
        <form className="utilityPageCard workoutRecordCard" aria-label="운동 기록" onSubmit={submitWorkout}>
          <header className="utilitySectionHeader">
            <strong>운동 기록</strong>
            <small>{profileSaved ? `${workoutKindText} · 저장된 프로필로 kcal 자동 계산` : '운동 프로필 저장 필요'}</small>
          </header>
...
export function AiNotePage({ navigate }) {
  const session = readStoredAuth();
  const noteBlocksKey = noteBlocksStorageKey(session);
...
export function NotionNotesPage({ navigate }) {
  const session = readStoredAuth();
  const noteBlocksKey = noteBlocksStorageKey(session);
  const noteBoardsKey = noteBoardsStorageKey(session);
  const routeTargetRef = useRef({
...
export function SchedulerPage({ navigate, embedded = false }) {
  const session = readStoredAuth();
  ...
  const submitDraft = (event) => {
    ...
    setItems((current) => [
      ...current,
      {
        ...
        repeatDays: draft.recurrence === 'custom' ? normalizeRepeatDays(draft.repeatDays, draft.date) : [],
      ...
      <section className="schedulerPage">
        ...
        {quickAddOpen ? <form className="schedulerQuickAdd cute" onSubmit={submitDraft}>
          ...
          {draft.recurrence === 'custom' ? (
            <fieldset className="schedule-custom-weekdays">
              <legend>반복 요일</legend>
          ...
          <div className="schedulerCalendarPanel">
            <div className="schedulerCalendarHeader">
              ...
              <div className="schedulerCalendarControls">
                <button type="button" onClick={() => moveCalendarMonth(-1)} aria-label="이전 달">{'<'}</button>
                <strong>{calendarMonth}</strong>
                <button type="button" onClick={() => moveCalendarMonth(1)} aria-label="다음 달">{'>'}</button>
              ...
              return (
                <article className={['schedulerItem', item.done ? 'done' : '', workoutItem ? 'workoutScheduleItem' : ''].filter(Boolean).join(' ')} key={item.id}>
                  <div className="schedulerNameCell">
                    <button
                      type="button"
                      className="schedulerCheck"
                      aria-label={`${item.title} ${item.done ? '대기로 되돌리기' : '완료로 표시'}`}
...
export function ConnectionsPage({ navigate }) {
  const session = readStoredAuth();
  return (
