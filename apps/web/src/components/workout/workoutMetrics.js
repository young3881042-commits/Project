...
export const WORKOUT_DURATION_OPTIONS = [15, 30, 45, 60];
...
export const WORKOUT_TEMPLATES = [
  ...
  },
  {
    ...
    items: ['직접 입력'],
    exercises: [
      { name: '기타 운동', sets: '3', reps: '10', weight: '' }
    ]
  }
];
...
export function bodyProfileWeightKg(profile) {
  const weightKg = Number(normalizeWorkoutProfile(profile).weightKg);
  if (
    !Number.isFinite(weightKg)
    || weightKg < BODY_PROFILE_LIMITS.weightKg.min
    || weightKg > BODY_PROFILE_LIMITS.weightKg.max
  ) return null;
  return weightKg;
}

export function calculateBodyBmi(profile) {
  const normalizedProfile = normalizeWorkoutProfile(profile);
  const weightKg = bodyProfileWeightKg(normalizedProfile);
  const heightCm = Number(normalizedProfile.heightCm);
  if (
    !weightKg
    || !Number.isFinite(heightCm)
    || heightCm < BODY_PROFILE_LIMITS.heightCm.min
    || heightCm > BODY_PROFILE_LIMITS.heightCm.max
  ) return null;
...
export function calculateWorkoutBmr(profile) {
  const normalizedProfile = normalizeWorkoutProfile(profile);
  const weightKg = Number(normalizedProfile.weightKg);
  const heightCm = Number(normalizedProfile.heightCm);
  const age = Number(normalizedProfile.age);
  if (
    !calculateBodyBmi(normalizedProfile)
    || !Number.isFinite(age)
...
export function workoutExerciseCalories(exercise, profile) {
  const weightKg = bodyProfileWeightKg(profile);
...
export function estimateWorkoutCalories({ templateId, durationMinutes, exercises, profile } = {}) {
  const weightKg = bodyProfileWeightKg(profile) || DEFAULT_WORKOUT_WEIGHT_KG;
