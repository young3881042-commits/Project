...
function calendarCellLabel(cell, summary) {
  ...
  if (summary.workoutPlanned) parts.push(`운동 ${summary.workoutPlanned}개 예정`);
  if (summary.workoutDone) parts.push(`운동 ${summary.workoutDone}개 완료`);
  return parts.join(', ');
}

function CalendarMarkers({ summary }) {
  const hasPlanned = summary.schedulePlanned > 0 || summary.workoutPlanned > 0;
  const hasDone = summary.scheduleDone > 0 || summary.workoutDone > 0;
  return (
...
function SelectedDateDetail({ date, detailRef, summary, navigate, today }) {
  ...
  const openActivity = (activity) => {
    if (activity.kind === 'workout') {
      navigate(`/workout?date=${encodeURIComponent(date)}`);
      return;
    }
    navigate(`/schedule?edit=${encodeURIComponent(activity.scheduleId || activity.id)}`);
  };
        ...
        <div className="homeMonthActivityList">
          {activities.map((activity) => (
            <button type="button" key={`${activity.kind}:${activity.id}`} onClick={() => openActivity(activity)}>
              <span className={`homeMonthActivityIcon ${activity.kind.startsWith('workout') ? 'workout' : ''}`}>
                <MemoNavIcon type={activity.kind.startsWith('workout') ? 'trophy' : 'calendar'} />
              </span>
              <span>
                ...
                <small>
                  ...
                  {activity.caloriesBurned ? ` · ${formatNumber(activity.caloriesBurned)}kcal` : ''}
                </small>
              </span>
              <em className={activity.status === '완료' ? 'done' : 'planned'}>({activity.status})</em>
            </button>
          ))}
          {remainingActivities.length ? (
            <div className="homeMonthRemainingActions">
              ...
              {hasRemainingWorkouts ? <button type="button" onClick={() => navigate(`/workout?date=${encodeURIComponent(date)}`)}>운동 전체 보기</button> : null}
            </div>
          ) : null}
        </div>
      ) : (
        <p className="homeMonthEmptyDetail">예정된 일정이나 완료한 운동이 없어요.</p>
      )}
    </section>
  );
}
  ...
  return (
    <section className="homeMonthCalendarCard" aria-labelledby="homeMonthCalendarTitle">
      ...
      <p className="homeMonthCalendarLegend">
        ...
        <span><strong>예정·완료</strong> 일정·운동 상태</span>
      </p>
