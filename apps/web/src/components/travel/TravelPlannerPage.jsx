...
function SelectedDestinations({ destinations, onRemove }) {
  if (!destinations.length) {
    return <p className="ltPlannerEmptySelection">선택하지 않아도 됩니다. 입력한 지역과 요청사항만 API로 보냅니다.</p>;
  }
...
function PlannerApiSummary({ preview }) {
  if (!preview) return null;

  return (
    <div className="ltPlannerApiCard" aria-label="API 전송 요약">
      <div>
        <span>API 전송</span>
        <strong>{preview.method} {preview.endpoint}</strong>
      </div>
      <dl>
        <div>
          <dt>지역</dt>
          <dd>{preview.region || '입력 필요'}</dd>
        </div>
        <div>
          <dt>날짜</dt>
          <dd>{preview.startDate || '미정'} · {preview.days}일</dd>
        </div>
        <div>
          <dt>기준</dt>
          <dd>{preview.quality}</dd>
        </div>
      </dl>
      <p>{preview.usesLocalCandidates ? '선택한 후보만 함께 보냅니다.' : '내부 기본 여행지 데이터는 보내지 않습니다.'}</p>
    </div>
  );
}
    ...
    return (
      <div className="ltGenerateStatus running" role="status" aria-live="polite">
        ...
        <div>
          ...
          <p>입력값을 API에 보내고, 시간표형 JSON 응답을 일정으로 저장합니다.</p>
    ...
    <main className="ltPage ltPlannerSimplePage">
      <section className="ltPageTitle">
        <div>
          ...
          <p>최소 입력을 API로 보내고 엑셀형 시간표 결과를 받습니다.</p>
        </div>
      </section>
      ...
      <section className="ltPlannerLayout ltPlannerSimpleLayout">
        ...
        <aside className="ltPlannerPreview">
          <PlannerApiSummary preview={apiPreview} />
