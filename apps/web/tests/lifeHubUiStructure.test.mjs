...
test('v55 경량 캐시는 거래 검색과 기존 생활 기록 화면을 포함한다', async () => {
  ...
  assert.match(serviceWorker, /const CACHE_NAME = 'orbit-web-v64'/);
...
test('Android WebView는 Orbit 이름·중앙 O 아이콘과 내장 웹 버전 갱신을 지원한다', async () => {
  ...
  assert.match(manifest, /android:versionName="0\.9\.5-debug"/);
  assert.match(manifest, /android:versionCode="46"/);
