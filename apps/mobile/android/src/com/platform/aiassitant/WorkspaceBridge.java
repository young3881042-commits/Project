...
final class WorkspaceBridge {
    ...
    synchronized void publish() throws Exception {
        if(destroyed)return;
        if(tree==null){token="";descriptor.delete();return;}
        ...
        if(!granted){tree=null;prefs.edit().remove(KEY).commit();token="";descriptor.delete();return;}
    ...
    private void result(String id,String error,boolean cancelled){activity.runOnUiThread(new Runnable(){ @Override public void run(){if(destroyed||!host.trusted())return;try{JSONObject data=new JSONObject().put("requestId",id).put("connected",tree!=null&&!token.isEmpty()).put("cancelled",cancelled);if(tree!=null)data.put("name",DocumentsContract.getTreeDocumentId(tree).replaceFirst("^[^:]*:",""));if(error!=null)data.put("error",error);host.deliver(data);}catch(Exception ignored){}}});}
    ...
    private void handle(Socket socket)throws Exception{
        ...
        JSONObject result;try{JSONObject body=new JSONObject(new String(bytes,StandardCharsets.UTF_8));String tool=body.getString("name");JSONObject args=body.optJSONObject("arguments");if(args==null)args=new JSONObject();
            ...
            if(!session.equals(token)||!selected.equals(tree))throw new IOException("폴더 연결이 바뀌었어요. 다시 요청해주세요.");
