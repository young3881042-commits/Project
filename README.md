...
메모·일정·가계부를 한 기기에서 관리하는 개인 생활 워크스페이스입니다. 생활 기록은 기존 브라우저 호환 캐시와 기기 내부 IndexedDB에 함께 보관하며 서버나 클라우드로 자동 전송하지 않습니다.
...
가계부에서는 월세·통신비·구독 같은 정기 결제를 이름, 금액, 결제일 또는 말일로 등록합니다. 이번 달 예정액과 실제 가계부 기록을 분리해 보여주며, 자동 반영은 같은 규칙·월에 한 번만 생성됩니다. 카드 알림 자동 가져오기를 함께 켜면 중복 가능성을 안내합니다. 일반 거래는 수정하거나 삭제 직후 되돌릴 수 있고, 월 예산은 사용자가 직접 설정한 경우에만 진행률을 표시합니다.
