...
export const workspaceTools = TOOL_NAMES.map(name => ({ name, description: ({list_files:'List entries in the default Orbit workspace (optional external/ mount) or any descendant. Use nextOffset to paginate.',read_file:'Read bytes of a workspace file as UTF-8 or base64. Repeat with nextOffset for large files. sha256 is available for files up to 1MB.',write_file:'Create or replace a file up to 1MB within the Orbit workspace. Existing files require expectedSha256 from a recent read. Never claim success if this tool returns an error.',create_directory:'Create one directory inside the Orbit workspace.',delete_entry:'Delete a file or empty directory only after the user confirms the exact path in the Android app. Never recursively delete a folder.'})[name],
  inputSchema: { type:'object', properties:{path:{type:'string',description:'Relative path; empty string denotes the Orbit workspace.'},...(name==='list_files'?{offset:{type:'integer',minimum:0}}:{}),...(name==='read_file'?{offset:{type:'integer',minimum:0},length:{type:'integer',minimum:1,maximum:65536}}:{}),...(['read_file','write_file'].includes(name)?{encoding:{type:'string',enum:['utf8','base64']}}:{}),...(name==='write_file'?{content:{type:'string'},expectedSha256:{type:'string'}}:{})}, required:name==='write_file'?['path','content']:['path'],additionalProperties:false } }));
