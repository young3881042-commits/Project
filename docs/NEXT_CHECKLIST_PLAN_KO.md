...
- 사용자 표시 이름은 `Orbit`으로 유지한다. 기존 Android package와 APK artifact의 `aiassitant`/`ai-assitant` 식별자는 별도 마이그레이션 전까지 호환용으로 유지한다.
...
- [x] 과거 공개 표기 오타 정리 이력을 남겼다. 현행 사용자 표시 이름은 `Orbit`이다.
