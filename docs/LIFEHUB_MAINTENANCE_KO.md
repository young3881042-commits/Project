...
- 완료 범위: 운동·식단 제외와 4탭 구조를 유지한 채, IndexedDB 호환 저장, 월별 정기 결제, 가계부 JSON/CSV 공유·거래 수정·월 예산, 날짜별 메모, 일반 텍스트 메모와 `orbit-web-v43`·`0.6.1-debug` APK까지 반영한다.
...
- 설치형 웹의 현재 서비스 워커 경계는 `orbit-web-v43`이다. 가계부 수동 입력 분리와 월별 카테고리 내역, 메인 상단 추가 버튼 제거, 오늘 날짜 상세의 안정된 일정·메모 버튼, 예산-지출 기준 월간 지갑을 포함하므로 기능 변경 없이 캐시 번호만 되돌리지 않는다.
