...
- 연결된 내장 Codex 채팅의 실행 권한은 workspace-write이며, 파일 변경은 기본 Orbit 작업공간과 기존 external/ 연결에서 native 파일 도구를 통해 수행한다. 허용된 5개 도구만 명시적 실행 승인 설정을 적용하고 다른 도구는 열지 않는다.
- 기본 파일 생성·읽기·수정은 바로 실행한다. 기존 파일의 해시 검사, 경로/심볼릭 링크 경계, 삭제·중요 파일의 native 확인은 유지한다. 도구의 성공 응답 전에 파일을 저장했다고 안내하지 않는다.

## 2026-09-11 채팅 사진 (현행)

- JPG/JPEG·PNG·WebP(원본 10MB까지)를 문서와 합계 3개까지 첨부한다. 사진은 전송용 JPEG로 축소하여 미리보기/대화에 보관하며 Codex의 실제 이미지 입력으로 전달한다. 최근 20개 메시지의 마지막 사진 3개까지 재사용하고 텍스트 검색/Markdown에는 사진 이름만 남긴다.
- 사진 원본 보관함 기능은 아니며 원본 파일은 그대로 둔다. 지원되지 않는 구버전 연결에는 업데이트 안내를 표시한다. 여행 탭은 복원하지 않는다.

## 2026-09-11 여행 보류·채팅 첨부 (현행)

- 여행 만들기 기능은 사용자 요청으로 보류한다. 하단 메뉴는 홈·일정·메모·가계부·AI 5개다. 저장 여행 데이터는 유지하되 여행 화면을 다시 노출하지 않는다.
- 채팅의 PDF·텍스트 문서는 전용 native picker로 읽고 앱 소유 임시 URL로 웹에 전달한다. WebView content/file 접근 차단을 완화하지 않는다. 기본 Orbit 작업공간과 기존 선택 폴더는 유지한다.

## 2026-09-11 기본 작업공간과 여행 PNG (현행)

- 내장 Codex 파일 도구는 앱 전용 `files/orbit-ai/workspace`를 기본으로 사용한다. 별도 저장소 권한 없이 활성화하며, 기존 SAF 외부 폴더는 `external/` 경로로 보존한다. 인증 파일은 작업공간 밖에 있으며 상대 경로·심볼릭 링크 범위 검사를 유지한다.
- 여행 화면은 날짜별 PNG 한 장만 표시한다. `travel/<일정 해시>.png`와 일정 JSON을 보관하고 동일 출처의 네이티브 파일 응답으로 불러온다. 실패한 지도 검색 때문에 일정 이미지까지 숨기지 않는다. 이전 IndexedDB 이미지도 보존·이전한다.
- 생성형 그림 도구를 새로 연결한 것은 아니며, AI가 만든 일정과 지도 타일을 PNG로 합성한다. 기본 작업공간은 앱 삭제 시 삭제되고 생활 기록 JSON 백업 대상이 아니므로 필요한 PNG는 외부 저장 기능으로 내보낸다.
