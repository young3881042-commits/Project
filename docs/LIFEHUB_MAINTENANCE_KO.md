...
- 완료 범위: 운동·식단 제외와 4탭 구조를 유지한 채, IndexedDB 호환 저장, 월별 정기 결제, 가계부 JSON/CSV 공유·거래 수정·월 예산, 날짜별 메모, 일반 텍스트 메모와 `orbit-web-v42`·`0.6.1-debug` APK까지 반영한다.
...
- 월 지갑 요약은 항상 유지하고 나머지는 `내역(입력·조회)`, `자동 기록(결제 알림·정기 결제)`, `분류·공유(분류 설정·파일)` 3개 내부 메뉴로 나눈다. 날짜 딥링크와 자동 기록의 수동 입력 버튼은 `내역`으로 돌아간다.
...
- `features/finance/FinanceSectionTabs.jsx`: 월 지갑 아래 3개 내부 메뉴와 좌우 방향키 이동을 담당하는 표시 컴포넌트
...
- 설치형 웹의 현재 서비스 워커 경계는 `orbit-web-v42`이다. 메인 상단 추가 버튼 제거, 오늘 날짜 상세의 안정된 일정·메모 버튼, 예산-지출 기준 월간 지갑과 기존 v38 기능을 포함하므로 기능 변경 없이 캐시 번호만 되돌리지 않는다.
