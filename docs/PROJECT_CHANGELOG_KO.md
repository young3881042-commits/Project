...
- 서버는 JPEG 바이트 형식·해상도·용량을 검사한다. 최근 20개 메시지 중 마지막 사진 3개를 임시 private JPEG로 기록하고 `codex exec --image`로 전달한다. 실행 종료·실패 후 임시 파일을 삭제하며 base64를 텍스트 프롬프트나 Markdown/FTS 본문에 넣지 않는다. 오래된 사진 중 실제로 포함되지 않은 것은 볼 수 없다고 안내하도록 지시한다.
- 채팅 메시지만 1MB 요청 범위 안에서 전송하며 chat 응답 한도 10MB·대화 저장 한도 8MB를 적용한다. 이미지 지원 capability를 확인해 구버전 서버에 사진을 보내지 않는다. Markdown 내보내기는 사진 이름만 포함하며 사진 자체는 앱 대화/JSON에 보관한다.
- 채팅·UI·서버·Codex 테스트 127개 통과. 실제 Codex에 합성 테스트 사진을 전달해 빨간 원·파란 정사각형·숫자 731을 인식하는 것을 확인했다. 사용자 개인 사진은 검증에 사용하지 않았다. Android 0.9.8-debug(49), 웹 캐시 v67. 웹 build·Android 컴파일·native smoke·서명·민감정보 검사 통과. /sdcard/Download/Orbit-latest.apk 복사 및 체크섬 일치 확인. 경량 SDK의 Android Lint와 실제 설치 후 사진 선택/전송 화면 왕복은 미실행이다.
- 입력 방식은 [OpenAI 공식 CLI 문서](https://learn.chatgpt.com/docs/developer-commands?surface=cli)와 설치된 exec 도움말로 확인했다. 여행 기능은 보류 상태를 유지한다.

## 2026-09-11 여행 탭 보류 · 채팅 문서 첨부 수정 · v48

- 요청에 따라 하단 여행 탭과 여행 화면 lazy route를 제거하고 기존 /travel 접근은 홈으로 처리한다. 하단은 홈·일정·메모·가계부·AI 5개이며 저장 여행 데이터와 백업은 유지한다.
- 채팅 첨부를 전용 native picker로 분리했다. WebView의 content URI 접근 차단을 유지하면서 사용자가 고른 문서를 네이티브에서 제한 크기만큼 읽고 임시 파일로 제공한다. URI나 저장소 권한을 웹에 넘기지 않는다. 동일 출처 임시 URL·실제 바이트 수 검증 후 기존 PDF/텍스트 추출·미리보기·메시지 저장 흐름으로 전달한다.
- 문서 제공자가 MIME 유형을 octet-stream으로 표시해도 선택할 수 있으며 지원 확장자·실제 읽기 크기에서 검증한다. 파일 선택 취소는 첨부 초안을 유지하고 형식/읽기 실패는 채팅에 표시한다. 파일 선택·첨부 중 자동 대화 선택으로 파일이 사라지는 경로도 막았다.
- 지원 형식은 PDF(5MB)·TXT/MD/Markdown/CSV/JSON/LOG(200KB), 최대 3개다. 사진·스캔 PDF·워드·엑셀 지원을 추가한 것은 아니다. 채팅·화면·서버 테스트 117개 통과. Android 0.9.7-debug(48), 웹 캐시 v66. 웹 build·Android 컴파일·native smoke·서명·민감정보 검사 통과. APK를 /sdcard/Download/Orbit-latest.apk에 복사하고 체크섬 일치를 확인했다. 경량 SDK의 Android Lint와 실제 설치 후 문서 제공자 선택/전송 왕복은 미실행이다.

## 2026-09-11 기본 Orbit 작업공간 · 일자별 PNG 파일 표시 · v47

- Codex 로컬 파일 도구의 기본 루트를 앱 전용 `files/orbit-ai/workspace`로 설정했다. 별도 저장소 권한·폴더 선택 없이 생성하며, 기존 SAF 지정 폴더는 `external/`에 보존한다. 일반 파일 읽기·생성·수정에는 추가 권한 창이 없고 기존 삭제·중요 파일 확인은 유지한다. 인증/런타임 파일은 작업공간 밖에 둔다.
- 여행 PNG와 일정 JSON을 `travel/<일정 해시>.png/json`에 기록한다. 파일을 기록한 뒤 네이티브에서 읽기·PNG 형식을 확인한 동일 출처 URL만 화면에 전달한다. 이미지 요청은 WebView와 ServiceWorker 양쪽에서 처리하고 SW 캐시에서 제외한다. 이전 IndexedDB 이미지는 읽을 때 파일로 옮기며 원본을 보존한다.
- 지도 위치/배경 조회 실패 때문에 만들어진 일정 이미지까지 숨기던 처리를 수정했다. 첫 이미지라면 확인 한계를 표시한 일정 PNG를 보여주고, 저장된 이미지가 있으면 재생성 실패 시 기존 파일을 유지한다. 여전히 앱이 지도 타일과 AI 일정을 합성하는 방식이며 별도 생성형 그림 모델을 실행하는 기능은 아니다.
- 여행·화면·대화·MCP 테스트 129개, Android 작업공간 파일 저장/교체/범위 검사, 웹 build·Android 컴파일·native smoke·서명·민감정보 검사 통과. 지도 조회 실패 조건의 실제 PNG도 생성해 확인했다. 경량 SDK의 Android Lint와 실제 APK 설치 후 화면 왕복은 미실행이다. APK를 /sdcard/Download/Orbit-latest.apk에 복사하고 원본과 SHA-256 일치를 확인했다.
- 작업공간은 앱 삭제 시 삭제되며 생활 기록 JSON 백업에는 포함되지 않는다. 필요한 여행 PNG는 이미지 저장 버튼으로 외부에 내보낸다. Android 0.9.6-debug(47), 웹 캐시 v65.

## 2026-09-10 여행 생성 연결 복구 · v46

- 내장 서버가 살아 있는데도 200ms 상태 확인이 실패하면 서버를 종료하던 경로를 제거했다. 실제 요청의 시간 제한은 유지하고 종료된 프로세스만 다시 시작해 진행 중인 메모리 작업이 짧은 지연으로 사라지지 않게 한다.
- 여행 진행 조회의 일시적 연결 오류·429·5xx는 1.8/3.6/7.2초 간격으로 최대 3회 재조회한다. 생성 요청은 재전송하지 않는다. 숨긴 화면은 조회를 멈추며 화면 복귀와 온라인 전환 시 이어서 조회한다. 인증 실패는 자동 반복하지 않고 안내를 유지한다.
- 여행·화면 테스트 115개 통과. 같은 생성 코드로 개인 데이터 없는 서울 종로 1일 일정을 실제 Codex에서 107초 만에 생성(7개 일정)했다. 이는 Termux 실행 경로 검증이며 앱 내부 계정·사용자가 본 오류의 원인까지 확인한 것은 아니다.
- Android 0.9.5-debug(46), 웹 캐시 v64. 웹 build·Android 컴파일·native smoke·서명 v1/v2/v3·민감정보 검사 통과. 경량 SDK의 Android Lint 및 실제 설치·화면 왕복은 미실행이다. APK를 /sdcard/Download/Orbit-latest.apk에 복사하고 원본과 체크섬 일치를 확인했다.

## 2026-09-10 날짜별 일정 이미지 한 장 · v45

- 날짜 탭 아래에 지도·방문 순서·시간표를 합친 PNG 한 장을 표시한다. 별도 대화형 지도와 중복 시간표를 제거했다. 지도 타일과 일정 데이터를 앱에서 이미지로 합성한다.
- 선택한 날짜를 처음 볼 때 이미지를 만들고 기기 IndexedDB에 보관한다. 같은 소유자·일정은 앱 재실행 시 저장본을 사용하며, 다시 만들기는 같은 키의 이미지 한 장을 교체한다. 다른 날짜의 지도는 미리 다운로드하지 않는다.
- 좌표 또는 지도 배경 조회 실패 시 기존 이미지를 보존한다. 저장 공간 부족은 화면에 안내하며 이미지 저장 버튼으로 PNG 파일을 내보낼 수 있다. 자동 저장 이미지는 기기 로컬 전용이며 JSON 백업에는 포함되지 않는다.
- 여행·화면 테스트 112개 및 웹 production build 통과. Android 0.9.4-debug(45), 웹 캐시 v63. Android 컴파일·native smoke·서명 v1/v2/v3·APK 민감정보 검사 통과. 경량 SDK의 Android Lint는 제외했다. APK는 /sdcard/Download/Orbit-latest.apk로 복사했으며 설치 후 실제 화면 조작은 사용자 확인이 필요하다.
...
## 2026-09-10 — 0.9.3-debug 실제 지도와 지정 폴더 접근

여행의 긴 시간표 이미지를 기본 화면에서 제거하고 Leaflet 지도로 교체했다. 확대·축소, 드래그,
전체 동선 맞춤, 장소 번호 팝업/장소 버튼 이동을 지원한다. 날짜별 시간표는 지도 아래 한 번만 표시한다.
지도 저장은 필요할 때만 실행하며 시간표를 제외한 지도 PNG를 만든다. 선은 방문 순서이고 도로 길찾기가 아니다.
Leaflet 스크립트·스타일·라이선스는 APK에 포함하고 지도 타일은 현재 화면에 맞춰 요청한다.

내장 Codex 연결 설정에서 Android ACTION_OPEN_DOCUMENT_TREE로 폴더의 읽기·쓰기 권한을 한 번 선택한다.
지정 폴더/하위 폴더를 DocumentsContract의 자식 탐색으로만 접근하며 임의 파일 경로/URI는 받지 않는다.
사용 가능한 도구는 list_files/read_file/write_file/create_directory/delete_entry 다섯 가지이다.
목록은 200개씩, 읽기는 한 번에 최대 64KB씩 나누어 큰 파일도 읽고 UTF-8/base64를 지원한다.
생성·수정은 파일당 1MB까지이며 기존 파일을 읽은 SHA-256을 요구하고 저장 후 재검증한다.
실패 시 이전 내용 복구를 시도하고, 복구 실패는 명확한 오류로 알린다.
삭제와 중요 설정/인증 파일 수정은 Android에서 구체적인 경로를 확인한 뒤 실행한다. 비어 있지 않은 폴더는
한 번에 삭제하지 않는다. 연결 변경/해제 및 채팅 취소는 기존 도구 세션을 무효화한다.

원본 파일은 복사 보관하지 않고 요청한 작업 때만 읽는다. 파일 내용은 AI 처리에 전달될 수 있다.
네이티브 루프백은 앱 전용 토큰/고정 도구 경로/Host 검사/Origin 거부/요청 크기 제한으로 보호한다.
토큰·폴더 URI는 React에 전달하지 않으며, 앱이 종료되면 연결 서버도 종료된다. 폴더 권한만 다음 실행에 유지한다.
셸/임의 MCP는 계속 차단하며 여행·결제 분류는 폴더 도구를 사용하지 않는다. Termux 모드에서는 이 SAF 연결을
사용하지 않으며 내장 AI로 전환해야 한다. Android가 선택을 허용하지 않는 시스템/다른 앱의 사유 영역은 접근할 수 없다.

검증: 웹 관련 테스트 120개, 서버 39개 통과. APK에 포함한 Android Node의 MCP 시작/도구 목록/인증 요청과
실제 Codex의 다섯 도구 인식을 확인했다. Android 파일 제공자의 실제 읽기·쓰기, 파일 선택 UI 및 지도 제스처는
설치 후 기기 확인이 필요하다. Android Lint는 이 환경의 기존 빌드 설정에 따라 생략했다.
참고: https://learn.chatgpt.com/docs/extend/mcp 및 https://leafletjs.com/download.html
