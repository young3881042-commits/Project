...
이 섹션은 이전 단계의 변경 기록이며, 이후 제품 방향은 아래 10절과 11절의 `/app` 앱 홈, `/portfolio`, `/connections` 기준으로 다시 정리했습니다.
...
- `/connections`: 개인 데이터 연결, 권한, 동기화 상태, 해제 액션을 관리하는 화면입니다.
...
- 실제 라우팅을 문서 기준과 맞춰 `/app` 앱 홈, `/portfolio` 포트폴리오, `/connections` 연결 화면으로 구현합니다.
